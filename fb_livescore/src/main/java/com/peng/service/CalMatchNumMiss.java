...
public class CalMatchNumMiss {
    ...
    public static void calculate() throws ParseException {
        ...
        while (lastDate.before(new Date())) {
            ...
            for (int i = 1; i <= 300; i++) {
                //获取昨天的数据
                matchNumBean = MatchNumRepository.findByLiveDateAndNum(lastDate, MyUtil.formatMatchNum(i));
                matchNumBean.setLiveDate(lastDate);
                matchNumBean.setMatchNum(MyUtil.formatMatchNum(i));

                MatchBean matchBean = matchBeans.get(MyUtil.formatMatchNum(i));
